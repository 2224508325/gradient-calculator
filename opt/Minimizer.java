...
public class Minimizer {
	...
	public void minimize(Polynomial p) throws Exception {
		if(p._terms.size() == 0) {
			throw new Exception("The polynomial is empty, or variables does not match");
		}
		if(!p.getAllVars().equals(_x0._hmVar2Value.keySet()))
			throw new Exception("Dim is not the same");

			
		
		// TODO: Build the partial derivatives of p for all variables in p
		//HashMap<String,Polynomial> a = new HashMap<String,Polynomial>(_var2gradp);
		_var2gradp = new HashMap<String,Polynomial>();
		for(String i : p.getAllVars()) {
			String var = i;
			_var2gradp.put(i, p.differentiate(var));
		}
		...
		while(_nIter < _maxIter && _lastGradNorm >_eps) {
			_nIter++;
			_lastx.setAll(_x0);
			remember = _lastGradNorm;
			double summ = 0;
			for(String j: _var2gradp.keySet()) {
				gradient._hmVar2Value.put(j,_var2gradp.get(j).evaluate(_lastx));
			}
			for(String j : gradient._hmVar2Value.keySet()) {
				summ += Math.pow(gradient._hmVar2Value.get(j), 2);
			}
			
			_lastGradNorm = Math.sqrt(summ);
			//if(_lastObjVal != 0 || _lastObjVal > p.evaluate(_x0))
			
			//last x = last x + negative gradient * stepsize
			//if(_lastGradNorm < remember) {
			
			for(String j : _lastx._hmVar2Value.keySet()) {
				_lastx._hmVar2Value.put(j,_lastx._hmVar2Value.get(j)-gradient._hmVar2Value.get(j)*_stepSize);
			}
			//_lastx = _lastx.sum(gradient.scalarMult(-_stepSize));
			
			//_stepSize = _stepSize/_nIter;
			//}
			//else if(_lastGradNorm > remember) {
			//	for(String j : _lastx._hmVar2Value.keySet()) {
			//		_lastx._hmVar2Value.put(j,_lastx._hmVar2Value.get(j)-gradient._hmVar2Value.get(j)*_stepSize);
			//	}
			//	_stepSize = _stepSize/_nIter;
			...
			System.out.format("At iteration %d: %s objective value = %.3f\n", _nIter, _lastx, _lastObjVal);	
			//_x0 = _x0 + _x0 * _stepSize
			
			//_lastObjVal = _lastGradNorm;
