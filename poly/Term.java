...
public class Term {
	...
	public Term(Term t) {
		_coef = t._coef;
		_vars = new ArrayList<String>(t._vars);
		_pows = new ArrayList<Integer>(t._pows);
	}
	...
	public double evaluate(Vector assignments) throws PolyException {
		...
		//if (_vars.size() != assignments._hmVar2Value.size()) 
		//	throw new PolyException("dimention not the same");
		//if(assignments._hmVar2Value.size() != _vars.size())
		//	throw new PolyException("variable is not the same");
		for(int i = 0;i < _vars.size();i++) {
			if (!assignments._hmVar2Value.containsKey(_vars.get(i)))
					throw new PolyException("variable not the same");
		}
		//ArrayList<Double> var = new ArrayList<Double>();
		double result = 1;
		//for (String i : assignments._hmVar2Value.keySet()) {
		//			var.add(assignments._hmVar2Value.get(i));
		//}
		for(String j : assignments._hmVar2Value.keySet())
		for (int i = 0; i < _vars.size(); i++) 
			if(_vars.get(i).equals(j))
				result *= Math.pow(assignments._hmVar2Value.get(j),(_pows.get(i)));
		
		double result2 = result*_coef;
		return result2;
	}
	...
	public Term differentiate(String var) {
		...
		Term tm = new Term(this);
		if(!this._vars.contains(var))
			return null;
		for(int i = 0;i < _vars.size();i++) {
			if(_vars.get(i).equals(var)) {
				tm._coef *= tm._pows.get(i);
				tm._pows.set(i,_pows.get(i)-1);
				if(tm._pows.get(i) == 0) {
					tm._vars.remove(i);
					tm._pows.remove(i);
	...
	public static void main(String[] args) throws Exception {
		...
		//term.differentiate("x");
		System.out.println(term.differentiate("x"));

		System.out.println(term.evaluate(vec1));
